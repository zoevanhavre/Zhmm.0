...
processHMMs<-function(Result, K=5,plotname="", simMeans=c(5,2,8)){    
        YZ<-Result$YZ
        filelabel<-plotname
        n<-length(YZ$Observed)

        TrimThin<-function(out, burn=500, newN=500){
                ...
                S<-out$States[ids,]
                MAP<-out$MAP[ids]
                return(list("Means"=MU, "Trans"=Q,"q0"=q0, "States"=S, "Y"=out$Y, "MAP"=MAP))
                }
        unswStates<-function( Out, K=K){
          # matrices to store 
            ...
            finMu<-Out$Means;finQ<-Out$Trans;finq0<-Out$q0;finS<-Out$States
            Zmap<-factor(Out$States[which.max(Out$MAP),], levels=1:K, ordered=FALSE)

            #MATCH REF TO TRUTH, FOR comparing later
         # match Zref to truth  ONLY TO MAKE SURE GROUP LABELS LINE UP TO COMPARE RESULTS, obviusly not in practice! this is fastest way.
          PermScore1<-function(x) {  Zref2<-Zmap;   levels(Zref2)<-c(1:K)[order(x)]; sum(as.numeric(as.character(Zref2[-1]))!=Out$Y$States)   }
          ZrefScore<- apply(perm, 1,PermScore1)       
          Zref.sw <-perm[which.min(ZrefScore),]  
          zz<-Zmap  ; levels(zz)<-c(1:K)[order(Zref.sw)]  ; Zref <-as.numeric(as.character(zz))   
          Zmap<-factor(Zref, levels=1:K, ordered=FALSE)
            ...
            for(i in 1:dim(Out$States)[1]){   #for each iteration
              ...
              permPARS<-c(1:K)[order(minPerm)]; finMu[i,]<-Out$Means[i, permPARS]; finq0[i,]<-Out$q0[i, permPARS]
            ...
            return(list("Means"=finMu, "Trans"=finQ,"q0"=finq0, "States"=finS, "Y"=Out$Y))}

        res.fit<-TrimThin(Result,burn=50, newN=200)
        res.us<-unswStates(Out=res.fit, K=K)
      
        # reorder to match truth
       ...
       # truSize<-table(YZ$States)/length(YZ$States)
       ...
       #compute means of q0, sd and ratio  
        ...
        # predicted:
          ...
          predZ<-  as.numeric(unlist(lapply(apply(ostates, 2,table), ordX)))
          RScore<-sum(yStates==predZ[-1])/length(yStates) 
          YZpred<-data.frame("Y"= YZ$Obs, "Zpred"=predZ[-1],"trueZ"=yStates )  # is this right or shud be -1?
    
          Zhat<-predZ
          Mhat<-apply(omeans, 2, mean)
          Qhat<-apply(oTrans, 2, mean)
          q0hat<-apply(oq0, 2, mean)
          all<-list("Zhat"=Zhat, "Mhat"=Mhat,"q0hat"=q0hat, "Qhat"=Qhat )

        return(list( "Zhat"=Zhat, "Mhat"=Mhat,"q0hat"=q0hat, "Qhat"=Qhat,  "RandScore"=RScore, "YZpred"=YZpred$Zpred))
          }
