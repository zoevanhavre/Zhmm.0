 #' gibbsHMM_PT
#'
#' HMM gibbs with PT, initial dist not stationary dist
...
gibbsHMM_PT_FlexIt<-function(YZ, M=2000, K=10, mu0=0, var0=100,alphaMAX=1, alphaMin=1e-10, J=20, lab="sim", type="First", InitType=c("PropObs", "UniIni")){
    ...
    varknown<-1 # known variace 
     ...
     # INITIALIZE
	startVal<-replicate(J, list())
    for(j in 1:J){startVal[[j]]<-makeStartSimpler(Y, K)}
          ...
          # FOR EACH CHAIN...
             # make matrix of alphas
                  ...
                  if(type=="First"){ 
                     ...
                     # 1.1  Transition matrix Q from Z(m-1) conditional posterior
                      	if (m==1) {nt<-CountTrans(startVal[[j]], K)
                       } else { nt<-CountTrans(Z[[j]][m-1,],K)}  
                          
                     # draw transition probs for state 1:K
                	qnew<-matrix(ncol=K, nrow=K)
                    for(k in 1:K){
                    qnew[k,]<-rdirichlet(par=nt[k,]+AllAlphas[k,])}
					q0new <-  ALTERNATEq0(qnew)  
                       
Q[[j]][m,]<-as.vector(t(qnew)) 							
q0[[j]][m,]<-q0new                       				
Qold[[j]]<-  matrix( Q[[j]][m,]  , K,K, byrow=TRUE)  	

                # 1.2 Update mu's    
                # compute needed values:  
				#N(k) = number of times state k is visited in chain, and sum(y_k) = sum of y's in state k 
                if (m==1) {sumNcount<-formu(startVal[[j]][-(n+1)],Y,K) 	#******
                } else {sumNcount<-formu(Z[[j]][m-1,-(n+1)], Y,K)}   #******
                   sumtot<-cbind(sumNcount$sumy, sumNcount$ny)
              # MEANS        
              mudraw<-apply(sumtot, 1,  function (x)     rnorm(1, mean= ((mu0/var0)+(x[1]/varknown)) / ((1/var0)+(x[2]/varknown)), sd= sqrt( 1/( (1/var0) + (x[2]/varknown)))  ))
              MU[[j]][m,]<-mudraw  
                
            # 2 Update States
          newZ<- UpdateStatesNewINIT( Y, Q[[j]][m,],  MU[[j]][m,],  m, sumNcount$ny, InitType)
          Z[[j]][m,]<-newZ$Z
...
## NEW, try each chain each time
 ...
 for( eachChain in 1:length(chainset)){
 	...
 	Alpha1<-matrix(Alpha_lows[j],ncol=K, nrow=K)
 	Alpha2<-matrix(Alpha_lows[j],ncol=K, nrow=K)
